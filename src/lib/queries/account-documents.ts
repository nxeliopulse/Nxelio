...
export interface AccountDocumentRecipient {
  id: string;
  name: string;
  email: string;
  signed: boolean;
...
export interface AccountDocumentRow {
  ...
  created_at: string;
  recipients: AccountDocumentRecipient[];
}

const BUCKET = "account-documents";
const MAX_FILE_BYTES = 15 * 1024 * 1024; // 15MB
const SELECT = "*, recipients:account_document_recipients(id, name, email, signed, signed_at)";

export async function getAccountDocuments(accountId: string): Promise<AccountDocumentRow[]> {
  ...
  return (data as unknown as AccountDocumentRow[]) || [];
}
 ...
 *  0111's migration comment). */
export async function createAccountDocument(accountId: string, formData: FormData): Promise<{ ok: boolean; error?: string }> {
  ...
  if (signatureRequired && validRecipients.length) {
    ...
    await supabase.from("account_document_recipients").insert(rows);
  }
...
export async function updateAccountDocumentStatus(id: string, accountId: string, status: DocStatus): Promise<{ ok: boolean; error?: string }> {
  const supabase = await createClient();
  const { error } = await supabase.from("account_documents").update({ status }).eq("id", id);
  if (error) return { ok: false, error: error.message };
  revalidatePath(`/accounts/${accountId}`);
  return { ok: true };
}
 ...
 *  e-signature (see the migration comment on 0111). */
export async function markRecipientSigned(recipientId: string, accountId: string, signed: boolean): Promise<{ ok: boolean; error?: string }> {
  const supabase = await createClient();
  const { error } = await supabase
    .from("account_document_recipients")
...
export async function deleteAccountDocument(id: string, accountId: string): Promise<{ ok: boolean; error?: string }> {
  const supabase = await createClient();
  const { error } = await supabase.from("account_documents").delete().eq("id", id);
  if (error) return { ok: false, error: error.message };
  revalidatePath(`/accounts/${accountId}`);
  return { ok: true };
}
