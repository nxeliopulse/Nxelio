...
export async function getAllSubscriptions(): Promise<SubscriptionRow[]> {
  ...
  const { data } = await admin
    .from("subscriptions")
    .select("workspace_id, plan_id, billing_interval, status, credits_remaining, credits_total, current_period_end, chargebee_customer_id, workspaces(name), subscription_plans(name)")
    .order("current_period_end", { ascending: true });

  return ((data as unknown as Array<{
    workspace_id: string; plan_id: string; billing_interval: string; status: string;
    credits_remaining: number; credits_total: number; current_period_end: string; chargebee_customer_id: string | null;
    ...
    chargebee_customer_id: r.chargebee_customer_id,
  }));
}
