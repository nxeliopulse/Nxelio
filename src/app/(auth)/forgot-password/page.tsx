...
function ForgotPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    // Previously the button was just disabled (`!email.includes("@")`) with
    // no message — QA report F01/F03: an empty or malformed email gave zero
    // feedback. Now it's always clickable and submit shows why it can't proceed.
    const trimmed = email.trim();
    if (!trimmed) { setError("Email is required."); return; }
    if (!isValidEmail(trimmed)) { setError(EMAIL_ERROR); return; }
    setEmail(trimmed);
    setError(null);
    setSending(true);
