...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ campaign?: string; q?: string }> }) {
  ...
  const [leads, stats, aiColumns, aiColumnSavedTemplates, users, hasReadyVerifiedLeads] = await Promise.all([
    getLeads(), getLeadStats(), getAiColumns(), getAiColumnSavedTemplates(), getUsers(), hasReadyLeadSearchJobs(),
  ]);
  ...
  if (campaign) {
    ...
    return <LeadsTable leads={filtered} stats={stats} campaignFilter={{ id: campaign, name: name || "Campaign" }} aiColumns={aiColumns} aiColumnSavedTemplates={aiColumnSavedTemplates} owners={owners} hasReadyVerifiedLeads={hasReadyVerifiedLeads} />;
  }

  return <LeadsTable leads={leads} stats={stats} initialSearch={q} aiColumns={aiColumns} aiColumnSavedTemplates={aiColumnSavedTemplates} owners={owners} hasReadyVerifiedLeads={hasReadyVerifiedLeads} />;
}
