...
export default async function LeadsPage({ searchParams }: { searchParams: Promise<{ campaign?: string; q?: string }> }) {
  ...
  if (campaign) {
    ...
    return <LeadsTable leads={filtered} stats={stats} campaignFilter={{ id: campaign, name: name || "Campaign" }} owners={owners} hasReadyVerifiedLeads={hasReadyVerifiedLeads} searchJobs={searchJobs} />;
  }

  return <LeadsTable leads={leads} stats={stats} initialSearch={q} owners={owners} hasReadyVerifiedLeads={hasReadyVerifiedLeads} searchJobs={searchJobs} />;
}
