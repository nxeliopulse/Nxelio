...
import { syncOutreachAccounts, hasConnectedLinkedIn } from "@/lib/queries/outreach-accounts";
...
export default async function OnboardingPage({
  ...
  // Every connect flow (mailbox, LinkedIn, Zoom, Calendar) returns here in
  // whatever tab it was opened in (usually a popup) — if that's THIS tab,
  // show a small confirmation instead of the full wizard. Rendering the
  // wizard here used to reset straight to step 1 (nothing is saved in this
  // tab's session), which is what made it look like connecting an
  // integration wiped all progress.
  if (sp.connected || sp.connect_error || sp.calendar_error) {
    // Email/LinkedIn (Unipile) connections land here without ever hitting one
    // of our own routes — this is the only place that persists them, so do it
  ...
  const [status, calendarProviderStatus, calendarAccounts, zoomConfigured, zoomAccounts, linkedinConnected] = await Promise.all([
    getOnboardingStatus(),
