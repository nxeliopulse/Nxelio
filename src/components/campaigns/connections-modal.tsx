...
export function ConnectionsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  function Row({ channel, label, hint, accts }: { channel: "linkedin" | "email"; label: string; hint: string; accts: OutreachAccountRow[] }) {
    ...
    return (
      <div className="rounded-xl border border-slate-200 p-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Button variant="outline" size="sm" onClick={() => connect(channel)} disabled={busy === channel}>
          ...
          <ul className="mt-3 space-y-1.5 border-t border-slate-100 pt-3">
            {accts.map((a) => (
              <li key={a.id} className="flex items-center justify-between gap-2 text-sm">
                ...
                <button onClick={() => disconnect(a.id)} disabled={busy === a.id} aria-label="Disconnect" className="p-1 rounded text-slate-300 hover:text-red-600 hover:bg-red-50">
    ...
    <Modal open={open} onClose={onClose} title="Connections" description="Connect the accounts campaigns send from" size="md">
      <div className="p-5 space-y-3">
        ...
        {loading ? (
          ...
          <>
            <Row channel="linkedin" label="LinkedIn" hint="Pull leads from search & posts, send invites/messages" accts={linkedin} />
            <Row channel="email" label="Email mailbox (optional)" hint="Send from your own inbox instead of the default sender" accts={email} />
