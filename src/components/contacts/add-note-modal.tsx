...
import { createContactNote } from "@/lib/queries/contact-notes";
 ...
 *  Real save (contact_notes), matching add-deal-modal.tsx's centered pattern. */
export function AddNoteModal({ open, onClose, contactId }: { open: boolean; onClose: () => void; contactId: string }) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = await createContactNote(contactId, formData);
