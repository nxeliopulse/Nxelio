...
import { createContactDocument } from "@/lib/queries/contact-documents";
...
export function AddDocumentModal({ open, onClose, contactId, owners, deals = [] }: { open: boolean; onClose: () => void; contactId: string; owners: OwnerOption[]; deals?: OpportunityRow[] }) {
  ...
  async function save() {
    ...
    try {
      ...
      const res = await createContactDocument(contactId, formData);
