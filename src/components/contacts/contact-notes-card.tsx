...
import { addContactNoteComment, deleteContactNote, updateContactNote, type ContactNoteRow } from "@/lib/queries/contact-notes";
 ...
 *  of the server-side sanitize in contact-notes.ts), and safely handles older
 *  plain-text notes that might contain literal "<"/">" characters. */
function safeHtml(html: string): string {
  return DOMPurify.sanitize(html, SANITIZE_OPTS);
}

/** Notes logged against a contact — with multiple file attachments and threaded
 *  replies per note (contact_note_files / contact_note_comments, 0099), and rich
 *  text bodies via the same TipTap editor used in the newsletter builder. */
export function ContactNotesCard({ contactId, notes }: { contactId: string; notes: ContactNoteRow[] }) {
  ...
  async function handleDelete(id: string) {
    ...
    if (!ok) return;
    start(async () => {
      await deleteContactNote(id, contactId);
    });
  }
    ...
    <div className="space-y-3 text-xs">
      ...
      {sorted.length === 0 ? (
        ...
        <ul className="space-y-3 pt-1">
          {sorted.map((n) => (
            <NoteItem key={n.id} note={n} contactId={contactId} onDelete={() => handleDelete(n.id)} />
          ))}
        </ul>
      )}

      <AddNoteModal open={addOpen} onClose={() => setAddOpen(false)} contactId={contactId} />
    </div>
  );
}

function NoteItem({ note, contactId, onDelete }: { note: ContactNoteRow; contactId: string; onDelete: () => void }) {
  ...
  function submitReply() {
    ...
    start(async () => {
      const res = await addContactNoteComment(note.id, contactId, reply.trim());
  ...
  function saveEdit() {
    start(async () => {
      const res = await updateContactNote(note.id, contactId, editBody);
