...
import { updateContactTask, updateContactTaskStatus, type ContactTaskRow } from "@/lib/queries/contact-tasks";
...
const PRIORITY_STYLES: Record<ContactTaskRow["priority"], string> = {
...
/** Upcoming (not-yet-done) tasks/reminders logged against a contact. New
 *  feature — no generic task system exists elsewhere in this app yet, so
 *  this is a simple, self-contained list scoped to a single contact. */
export function ContactTasksCard({
  contactId,
  tasks,
  owners,
}: {
  ...
  function markDone(id: string) {
    start(async () => {
      const res = await updateContactTaskStatus(id, contactId, "done");
      if (!res.ok) toast(res.error || "Couldn't update task", "error");
    });
  }

  function patch(id: string, fields: Partial<Pick<ContactTaskRow, "reminder" | "priority" | "assigned_to">>) {
    start(async () => {
      const res = await updateContactTask(id, contactId, fields);
      if (!res.ok) toast(res.error || "Couldn't update task", "error");
    });
  }
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 p-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      value={t.priority}
                      disabled={pending}
                      onChange={(e) => patch(t.id, { priority: e.target.value as ContactTaskRow["priority"] })}
                    >
