...
/** Real email send + history for a contact — gated on a connected mailbox
 *  (same one Settings > Email uses), since sending real email needs a real
 *  sender account. Reuses inbox_messages (0102 added contact_id, 0105 added
 *  to_email). Compose/draft/delete all happen in ComposeEmailModal. */
export function ContactEmailCard({
  contactId, contactEmail, emails, mailboxConnected,
}: {
  contactId: string;
  ...
  const [editingDraft, setEditingDraft] = useState<ContactEmailRow | null>(null);
  ...
  function openDraft(m: ContactEmailRow) {
    setEditingDraft(m);
    setComposeOpen(true);
  }
