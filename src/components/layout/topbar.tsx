...
import {
  Search, ChevronDown, LogOut, User as UserIcon, Settings, Menu, Sparkles,
  Plus, Phone, ShoppingBag, HelpCircle, ArrowUpRight
} from "lucide-react";
...
export function Topbar({ userName = "Guest", userEmail = "", onToggleAssistant, assistantOpen = false }: TopbarProps) {
  ...
  return (
    <header className="h-14 bg-[#1e242b] border-b border-[#2d3540] px-3 sm:px-4 lg:px-5 flex items-center justify-between gap-3 sticky top-0 z-30 text-slate-200 shadow-sm">
      {/* Left side: hamburger (mobile) + search ("Find or Ask") + Quick + */}
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        {/* Search ("Find or Ask") + Quick create button */}
        <div className="hidden sm:flex items-center gap-2 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-8 pl-8 pr-3 bg-[#2b323c] border border-[#3a4451] rounded-full text-xs text-white placeholder-slate-400 focus:outline-none focus:bg-[#343d49] focus:border-blue-500 transition-colors"
            />
          </div>
          <button
            onClick={() => router.push("/leads")}
            title="Quick add lead or campaign"
            className="h-8 w-8 rounded-full bg-[#2b323c] hover:bg-[#3a4451] border border-[#3a4451] flex items-center justify-center text-slate-300 hover:text-white transition-colors flex-shrink-0"
          >
