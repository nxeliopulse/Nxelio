  ...
  return (
    <div className="space-y-3">
      ...
      <Button variant="outline" className="w-full" onClick={connect} disabled={connecting || !unipileConfigured}>
        ...
        {accounts.length > 0 ? "Connect another mailbox (switch)" : "Connect mailbox"}
        {!connecting && <ExternalLink className="h-3.5 w-3.5 opacity-60" />}
      </Button>
      {!unipileConfigured && (
        <p className="text-xs text-slate-400">Mailbox connection needs Unipile configured (UNIPILE_DSN + UNIPILE_API_KEY).</p>
      )}
    </div>
  );
}
