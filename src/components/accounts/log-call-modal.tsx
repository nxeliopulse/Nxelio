...
import { createAccountCall } from "@/lib/queries/account-calls";
...
/** Log a call against an account — real data, same centered-modal pattern as AddDealModal. */
export function LogCallModal({ open, onClose, accountId }: { open: boolean; onClose: () => void; accountId: string }) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const res = await createAccountCall(accountId, {
        outcome,
