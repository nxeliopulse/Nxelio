...
/** Quick "schedule a meeting" modal for the Account detail page — same shape as
 *  AddDealModal, just posting into meetings via scheduleMeeting() with
 *  account_id set instead of lead_id. Doesn't send email invites by default;
 *  this is an internal scheduling action, not attendee notification. An
 *  Account has no email of its own, so there's no attendees-by-email list
 *  here (unlike the Contact version). */
export function ScheduleMeetingModal({
  ...
  const [title, setTitle] = useState(`Meeting with ${accountName}`);
