...
import { deleteAccountCall, updateAccountCallOutcome, type AccountCallRow } from "@/lib/queries/account-calls";
...
/** Real call log per account (account_calls, 0110) — outcome + notes + when,
 *  logged by whoever recorded it. Fills the "Calls" tab with real data. */
export function AccountCallsCard({ accountId, calls, onAddNew }: { accountId: string; calls: AccountCallRow[]; onAddNew: () => void }) {
  return (
    <div className="space-y-3">
      ...
      {calls.length === 0 ? (
        <p className="text-xs text-slate-400 italic text-center py-6">No calls logged for this account yet.</p>
      ) : (
        <ul className="space-y-3">
          {calls.map((c) => (
            <CallItem key={c.id} call={c} accountId={accountId} />
          ))}
        </ul>
      )}
    </div>
...
function CallItem({ call, accountId }: { call: AccountCallRow; accountId: string }) {
  ...
  function changeOutcome(outcome: CallOutcome) {
    start(async () => {
      const res = await updateAccountCallOutcome(call.id, accountId, outcome);
  ...
  async function handleDelete() {
    ...
    if (!ok) return;
    start(async () => {
      await deleteAccountCall(call.id, accountId);
    });
  }
