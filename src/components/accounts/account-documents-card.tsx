...
import { deleteAccountDocument, markRecipientSigned, updateAccountDocumentStatus, type AccountDocumentRow } from "@/lib/queries/account-documents";
...
/** Real document tracker (account_documents, 0111) — upload an existing
 *  quote/proposal/contract and track title/type/status/owner. No template
 *  builder or PDF generation. */
export function AccountDocumentsCard({ accountId, documents, owners, onAddNew }: { accountId: string; documents: AccountDocumentRow[]; owners: AccountOwnerOption[]; onAddNew: () => void }) {
    ...
    <div className="space-y-3">
      ...
      {documents.length === 0 ? (
        ...
        <ul className="space-y-2">
          {documents.map((d) => (
            <DocumentItem key={d.id} doc={d} accountId={accountId} ownerName={ownerName(d.owner_id)} />
          ))}
        </ul>
      )}
    </div>
...
function DocumentItem({ doc, accountId, ownerName }: { doc: AccountDocumentRow; accountId: string; ownerName: string }) {
  ...
  function changeStatus(status: DocStatus) {
    start(async () => {
      const res = await updateAccountDocumentStatus(doc.id, accountId, status);
  ...
  function toggleSigned(recipientId: string, signed: boolean) {
    start(async () => {
      const res = await markRecipientSigned(recipientId, accountId, signed);
  ...
  async function handleDelete() {
    ...
    if (!ok) return;
    start(async () => {
      await deleteAccountDocument(doc.id, accountId);
    });
  }
